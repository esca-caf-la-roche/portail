...
function CartePersonne({ personne, onOuvrirFusion }: { personne: PersonneAValider; onOuvrirFusion: (a: CandidatFusion, b: CandidatFusion) => void }) {
  ...
  async function associer(licence: string, confirmerIdentite = false) {
    if (!confirmerIdentite) setConfirmationIdentite(null);
    setMsg("Association…");
    try {
    ...
    <li className="abo-admin-card abo-admin-licence-card">
      ...
      {confirmationIdentite && (
        <div className="abo-admin-notice abo-admin-notice--warning" role="alert">
          <strong>
            Vérifiez l&apos;identité avant d&apos;associer la licence {confirmationIdentite.licence}
          </strong>
          <p>
          ...
          <p>
            Confirmez uniquement s&apos;il s&apos;agit bien de la même personne. Seuls le numéro et le statut de la licence seront modifiés ; le nom et le prénom du dossier seront conservés.
          </p>
          <div className="abo-admin-toolbar">
            <button
              type="button"
